...
| LED state save/restore | **SOLVED** | No disk state needed. Number LEDs colored via DATA_NOTIFY(8) on mute; restored to firmware colors (selected=green, unselected=white) on unmute/exit. |
...
6. **Application** — DONE. Built as native Rust app (Focusmute) with:
   ...
   - **LED restore** — reads `selectedInput` on unmute/exit, restores firmware colors (selected=green, unselected=white) via DATA_NOTIFY(8). No disk state needed.
